...
| `T1.3` Fixtures canonicos | Golden tests | lectura de `fixtures.manifest.json` + subset match |
...
| `T4.1` Pantalla unica | E2E smoke | carga JSON y submit exitoso |
| `T4.2` Visualizacion | UI tests/smoke | render de factible/infactible |
| `T4.3` Exportacion | UI tests/smoke | descarga JSON/CSV valida |
| `T5.1` CI | Pipeline checks | lint+test+build bloquean merge |
| `T5.2` Hardening | Regression suite | tiny + medium smoke verde |
